...
produce(edm::Event& iEvent, const edm::EventSetup& iSetup) {
  ...
  */
  //-------------------------------------------------------------------
  //Triggers to be saved. All events are saved, but only following triggers will be saved with the events (if they are satisfied).
  ...
  myTriggerName.push_back("HLT_CaloJet500_NoJetID_");
  myTriggerName.push_back("HLT_DiPFJetAve500_");
  myTriggerName.push_back("HLT_PFHT750_4JetPt50_");
  myTriggerName.push_back("HLT_PFHT800_");
  myTriggerName.push_back("HLT_PFJet450_");

  const int nmyTriggers=myTriggerName.size();
  ...
  if(triggerResults.isValid()) {
	...
	if (dataSource.empty() || std::find(dataSource.begin(), dataSource.end(), names.triggerName(i)) != dataSource.end()) {
	  ...
	  ///////////////////////////////////////////////////////////////////////////////
	  std::string temp=names.triggerName(i);
	  temp.pop_back();
	  temp.pop_back();
	  //      std::cout<<temp<<":"<<triggerResults->accept(i)<<std::endl;                                                                                       
	  if(temp == myTriggerName[p]) {
	    //      TrMatch=true;
	    v_hlt_insideDataset_prescales->push_back ( hltConfig.prescaleValue(hltPrescaleProvider.prescaleSet(iEvent,iSetup),names.triggerName(i)) );
	    //      v_hlt_insideDataset_names->push_back ( names.triggerName(i) );                                                                  
	    v_hlt_insideDataset_decisions->push_back ( triggerResults->accept(i) );
	    //      std::cout<<"acc:"<<triggerResults->accept(i)<<" "<<names.triggerName(i)<<std::endl;
	    break;
	  }
	  ////////////////////////////////////////////////////////////////////////////////////////  
	} else {
	...
	if(abs(i)==(triggerResults->size())-1){
	  v_hlt_insideDataset_prescales->push_back (hltConfig.prescaleValue(hltPrescaleProvider.prescaleSet(iEvent,iSetup),names.triggerName(i)));
	  //      v_hlt_insideDataset_names->push_back ( names.triggerName(i) );
	  //      v_hlt_insideDataset_names->push_back ("NoneTrg" );
	  v_hlt_insideDataset_decisions->push_back (false);
	}      
      }
    }
